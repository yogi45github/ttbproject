...
class Frontend extends Template
{
    ...
    public function getPostInfo($post)
    {
        ...
        $html           = __(
            '<span class="blog-post-date"> <i class="mp-blog-icon mp-blog-calendar-times"></i> %1 </span>',
            $this->getDateFormat($post->getPublishDate())
        );

        if ($categoryPost = $this->getPostCategoryHtml($post)) {
            $html .= __('<span class="blog-category"> Posted in %1 </span>', $categoryPost);
        }
        ...
        if ($author && $author->getName() && $this->helperData->showAuthorInfo()) {
            ...
            $html .= __('<span class="blog-author"> <i class="mp-blog-icon mp-blog-user"></i> %1 </span>', $aTag);
        }

        if ($this->getCommentinPost($post)) {
            $html .= __(
                '<span class="blog-comments"> <i class="mp-blog-icon mp-blog-comments" aria-hidden="true"></i> %1 </span>',
        ...
        if ($post->getViewTraffic()) {
            $html .= __(
                '<sapn class="blog-views">  <i class="mp-blog-icon mp-blog-traffic" aria-hidden="true"></i> %1 </sapn>',
                $post->getViewTraffic()
            );
        }

        if ($couldLike > 0) {
            $html .= __('<span class="blog-thumbs-up"> <i class="mp-blog-icon mp-blog-thumbs-up" aria-hidden="true"></i> %1 </span>', $couldLike);
        }
